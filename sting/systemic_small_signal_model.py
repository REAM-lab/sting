...
class Composite_small_signal_model:
    ...
    def build_generators_composite_model(self):
        ...
        for typ in list_of_gentypes:  # iterate over the list of types
            ...
            if list_of_gens:  # check if the list if empty.
                ...
                matrix1 = np.kron(
                    np.eye(n), np.hstack((np.eye(nd), np.zeros((nd, ng))))
                )
                matrix2 = np.kron(
                    np.eye(n), np.hstack((np.zeros((ng, nd)), np.eye(ng)))
                )
                ...
                dev_inputs = [
                    (g.idx, ud) for g in list_of_gens for ud in g.ssm.device_side_inputs
                ]
                grid_inputs = [
                    (g.idx, ug) for g in list_of_gens for ug in g.ssm.grid_side_inputs
                ]
                ...
                initial_device_side_inputs = np.vstack(
                    [g.ssm.initial_device_side_inputs for g in list_of_gens]
                )
                initial_grid_side_inputs = np.vstack(
                    [g.ssm.initial_grid_side_inputs for g in list_of_gens]
                )
                initial_outputs = np.vstack(
                    [g.ssm.initial_outputs for g in list_of_gens]
                )

                # Append the State_space_model object to the list
                ssm_of_gentypes.append(
                    State_space_model(
                        A=Astack,
                        B=Bnew,
                        C=Cstack,
                ...
                T2_of_gentypes.append(
                    np.hstack((np.eye(n * nd), np.zeros((n * nd, n * ng))))
                )
                T3_of_gentypes.append(
                    np.hstack((np.zeros((n * ng, n * nd)), np.eye(n * ng)))
                )
        ...
        initial_device_side_inputs = np.vstack(
            [ssm.initial_device_side_inputs for ssm in ssm_of_gentypes]
        )
        initial_grid_side_inputs = np.vstack(
            [ssm.initial_grid_side_inputs for ssm in ssm_of_gentypes]
        )
        initial_outputs = np.vstack([ssm.initial_outputs for ssm in ssm_of_gentypes])

        self.generators = State_space_model(
            A=Astack,
            B=Bnew,
            C=Cstack,
            D=Dnew,
    ...
    def build_shunts_composite_model(self):
        ...
        for typ in list_of_shunttypes:  # iterate over the shunt types
            ...
            if list_of_shunts:  # check if the list is empty
                ...
                grid_inputs = [
                    (s.idx, ug) for s in list_of_shunts for ug in s.ssm.grid_side_inputs
                ]
                ...
                initial_states = np.vstack(
                    [s.ssm.initial_states for s in list_of_shunts]
                )
                initial_grid_inputs = np.vstack(
                    [s.ssm.initial_grid_side_inputs for s in list_of_shunts]
                )
                initial_outputs = np.vstack(
                    [s.ssm.initial_outputs for s in list_of_shunts]
                )

                ssm_of_shunttypes.append(
                    State_space_model(
                        A=Astack,
                        B=Bstack,
                        C=Cstack,
        ...
        initial_grid_inputs = np.vstack(
            [ssm.initial_grid_side_inputs for ssm in ssm_of_shunttypes]
        )
        initial_outputs = np.vstack([ssm.initial_outputs for ssm in ssm_of_shunttypes])

        self.shunts = State_space_model(
            A=Astack,
            B=Bstack,
            C=Cstack,
            D=Dstack,
        ...
        print(
            f"  > Creation of files with the small signal model in {outputfolder_directory}: "
        )
    ...
    def build_branches_composite_model(self):
        ...
        for typ in list_of_branchtypes:
            ...
            if list_of_branches:
                ...
                grid_inputs = [
                    ...
                    for ug in b.ssm.grid_side_inputs
                ]
                ...
                initial_states = np.vstack(
                    [b.ssm.initial_states for b in list_of_branches]
                )
                initial_grid_inputs = np.vstack(
                    [b.ssm.initial_grid_side_inputs for b in list_of_branches]
                )
                initial_outputs = np.vstack(
                    [b.ssm.initial_outputs for b in list_of_branches]
                )

                ssm_of_branchtypes.append(
                    State_space_model(
                        A=Astack,
                        B=Bstack,
                        C=Cstack,
        ...
        initial_grid_inputs = np.vstack(
            [ssm.initial_grid_side_inputs for ssm in ssm_of_branchtypes]
        )
        initial_outputs = np.vstack([ssm.initial_outputs for ssm in ssm_of_branchtypes])

        self.branches = State_space_model(
            A=Astack,
            B=Bstack,
            C=Cstack,
            D=Dstack,
    ...
    def build_system_composite_model(self):
        ...
        gen_cx = graph_matrices.build_generation_connection_matrix(
            num_buses, gen_bus_connections
        )
        ...
        for typ in list_of_branchtypes:
            list_of_branches = getattr(self.input_system, typ)
            branch_frombus_tobus.extend(
                [(b.from_bus, b.to_bus) for b in list_of_branches]
            )

        # Build oriented incidence matrix
        or_inc = graph_matrices.build_oriented_incidence_matrix(
            num_buses, branch_frombus_tobus
        )
        ...
        d_gen = len(
            self.generators.device_side_inputs
        )  # total number of device_side_inputs of generators
        g_gen = len(
            self.generators.grid_side_inputs
        )  # total number of grid_side_inputs of generators
        y_gen = len(
            self.generators.outputs
        )  # total number of outputs of the generators

        g_br = len(
            self.branches.grid_side_inputs
        )  # total number of grid_side_inputs of branches
        y_br = len(self.branches.outputs)  # total number of outputs of branches

        g_sh = len(
            self.shunts.grid_side_inputs
        )  # total number of grid_side_inputs of generators
        ...
        F42 = np.kron(
            0.5
            * (
                np.kron(np.transpose(un_inc), np.array([[1], [1]]))
        ...
        ssm.outputs = (
            self.generators.outputs + self.shunts.outputs + self.branches.outputs
        )
        ...
        ssm.initial_states = np.vstack(
            (
                self.generators.initial_states,
                self.shunts.initial_states,
                self.branches.initial_states,
            )
        )
        ssm.initial_outputs = np.vstack(
            (
                self.generators.initial_outputs,
