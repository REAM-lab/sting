...
class State_space_model:
    ...
    initial_inputs: np.ndarray = field(
        default_factory=lambda: np.array([]).reshape(-1, 1)
    )
    initial_outputs: np.ndarray = field(
        default_factory=lambda: np.array([]).reshape(-1, 1)
    )
    initial_states: np.ndarray = field(
        default_factory=lambda: np.array([]).reshape(-1, 1)
    )
    device_side_inputs: list = field(default_factory=list)
    grid_side_inputs: list = field(default_factory=list)
    initial_device_side_inputs: np.ndarray = field(
        default_factory=lambda: np.array([]).reshape(-1, 1)
    )
    initial_grid_side_inputs: np.ndarray = field(
        default_factory=lambda: np.array([]).reshape(-1, 1)
    )
    ...
    def __post_init__(self):
        ...
        if (
            self.initial_device_side_inputs.size > 0
            or self.initial_grid_side_inputs.size > 0
        ):
            self.initial_inputs = np.vstack(
                (self.initial_device_side_inputs, self.initial_grid_side_inputs)
            )


def connect_models_via_CCM(
    F, G, H, L, components: list[State_space_model], inputs=None, outputs=None
):
